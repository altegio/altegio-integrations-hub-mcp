...
- New Marketplace installations copy the application's schedule webhook flag; existing installations retain their stored flags until updated. Schedule changes emit update events with empty data.
- The location hooks POST replaces every URL and all event flags. The updated GET exposes actual per-URL settings, while legacy top-level flags still describe the first URL. Use the webhook resource and review the full plan before changing it; there is no owner delivery-log/replay/test API.
