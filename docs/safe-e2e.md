...
8. Call `marketplace_get_installation_status` and verify `active`. Retrieve the application token through the protected Developer Cabinet, then call one harmless Business Management API read using `Authorization: Bearer <partner>, User <application_token>` outside this server, or use Altegio Pro MCP. Verify callback delivery in the application backend.
